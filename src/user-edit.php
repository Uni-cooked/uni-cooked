...
if(!isset($_POST["submit-profile-changes"]) && !isset($_POST["submit-change-psw"])) {
    ...
    if(isset($_FILES["profile-img-edit"]) && $_FILES["profile-img-edit"]["error"]==0) {
        ...
        if(mb_strlen($extension)==0 || !in_array($extension,$extensionArray)) {
            ...
            $errorFound=true;
            $paginaHtml = str_replace("{{profile-pic-error}}",'<p role="alert" id="profile-pic-err-p" class="err-msg">Il file caricato non è supportato</p>',$paginaHtml);
        } elseif($info->getSize()>1572864) {
            $errorFound=true;
            $paginaHtml = str_replace("{{profile-pic-error}}",'<p role="alert" id="profile-pic-err-p" class="err-msg">Il file caricato supera gli 1,5 <span lang="en">megabytes</span>: carica un file di dimensione minore</p>',$paginaHtml);
        } elseif(strcmp(mime_content_type($tmpFile),"image/jpeg")!=0 && strcmp(mime_content_type($tmpFile),"image/png")!=0) {
            $errorFound=true;
            $paginaHtml = str_replace("{{profile-pic-error}}",'<p role="alert" id="profile-pic-err-p" class="err-msg">Il file caricato è un formato non supportato</p>',$paginaHtml);
        } else {
    ...
    if(strcmp($oldPsw,"")==0) {
        $paginaHtml=str_replace("{{messaggio di old-psw}}",'<p role="alert" class="err-msg">Inserisci la <span lang="en">password</span> attuale</p>',$paginaHtml);
        $errorFound=true;
    }
    if(strcmp($newPsw,"")==0) {
        ...
        $errorFound=true;
        $paginaHtml = str_replace("{{messaggio di new-psw}}",'<p role="alert" class="err-msg">La <span lang="en">password</span> deve avere una lettera maiuscola, una lettera minuscola, un numero e un carattere speciale</p>',$paginaHtml);
    } else {
        $paginaHtml=str_replace("{{messaggio di new-psw}}","",$paginaHtml);
    }
    if(strcmp($newPsw,$repPsw)!=0) {
        $paginaHtml=str_replace("{{messaggio di repeat-psw}}",'<p role="alert" class="err-msg">La nuova <span lang="en">password</span> e la sua ripetizione non coincidono</p>',$paginaHtml);
        $errorFound=true;
    } else {
        $paginaHtml=str_replace("{{messaggio di repeat-psw}}","",$paginaHtml);
    }
    ...
    if(is_string($result) && strcmp($result,"wrongPassword")==0) {
        $paginaHtml=str_replace("{{messaggio di old-psw}}",'<p role="alert" class="err-msg">La <span lang="en">password</span> attuale non è corretta</p>',$paginaHtml);
        echo $paginaHtml;
        exit();
