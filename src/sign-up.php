...
if (isset($_POST['submit'])) {
    ...
    if (!isset($_POST['categoria'])) {
        $errorFound=true;
        $paginaHtml = str_replace("{{messaggio di categoria}}",'<p role="alert" class="err-msg">Seleziona una categoria</p>',$paginaHtml);
    ...
    if (mb_strlen($email)==0) {
        ...
        $errorFound=true;
        $paginaHtml = str_replace("{{messaggio di email}}",'<p role="alert" class="err-msg">L\'<span lang="en">email</span> non è un indirizzo valido</p>',$paginaHtml);
    } else {
        ...
        if (strcmp($isEmailPresent,"ExceptionThrow")!=0 && strcmp($isEmailPresent,"ConnectionFailed")!=0 && $isEmailPresent==true) {
            $errorFound=true;
            $paginaHtml = str_replace("{{messaggio di email}}",'<p role="alert" class="err-msg">Questa <span lang="en">email</span> non può essere utilizzata</p>',$paginaHtml);
    ...
    if (mb_strlen($psw)==0) {
        ...
        $errorFound=true;
        $paginaHtml = str_replace("{{messaggio di psw-ripetuta}}",'<p role="alert" class="err-msg">Le <span lang="en">password</span> non coincidono</p>',$paginaHtml);
    } else {
        $paginaHtml = str_replace("{{messaggio di psw}}","",$paginaHtml);
        $paginaHtml = str_replace("{{messaggio di psw-ripetuta}}","",$paginaHtml);
    }
