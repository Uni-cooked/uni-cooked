...
if (isset($_POST['submit'])) {
    ...
    if (mb_strlen($username)==0) {
        ...
        if (strcmp($isUserPresent,"ExceptionThrow")!=0 && strcmp($isUserPresent,"ConnectionFailed")!=0 && $isUserPresent==true) {
            $errorFound=true;
            $paginaHtml = str_replace("{{messaggio di nome}}",'<p aria-live="assertive" aria-atomic="true" class="err-msg">Il nome utente inserito non può essere utilizzato</p>',$paginaHtml);
    ...
    if (!isset($_POST['categoria'])) {
        ...
        $errorFound=true;
        $paginaHtml = str_replace("{{messaggio di categoria}}",'<p aria-live="assertive" aria-atomic="true" class="err-msg">La categoria inserita non è valida</p>',$paginaHtml);
    }
