    ...
    if($isUserLogged!=false)
    {
        ...
        if(is_string($userComment) && strcmp($userComment,"userLeftNoComment")==0) {
            ...
            if(isset($_SESSION["commentError"])) {
                $form.='<form method="post" action="recipe.php?recipe='.urlencode($recipe).'" class="content" id="first-form">'.'<fieldset><div class="input-container"><label class="form-label" for="add-comment-eval">Valuta questa ricetta (voto da 1 a 30)</label><div id="add-comment-eval-container"><input type="number" id="add-comment-eval" min="1" max="30" placeholder="18" required="required" name="mark"/><span><abbr title="su">/</abbr> 30</span></div></div><label class="form-label" for="add-comment-text">Commenta</label><textarea id="add-comment-text" maxlength="200" placeholder="Motiva la tua valutazione" required="required" name="comment"></textarea><p class="err-msg">'.$_SESSION["commentError"].'<button class="button-input button-input-confirm" type="submit" name="submit-add-review">VALUTA</button><button class="button-input button-input-cancel" type="reset">CANCELLA</button></fieldset></form></div>';
                unset($_SESSION["commentError"]);
            } else {
                $form.='<form method="post" action="recipe.php?recipe='.urlencode($recipe).'" class="content" id="first-form">'.'<fieldset><div class="input-container"><label class="form-label" for="add-comment-eval">Valuta questa ricetta (voto da 1 a 30)</label><div id="add-comment-eval-container"><input type="number" id="add-comment-eval" min="1" max="30" placeholder="18" required="required" name="mark"/><span><abbr title="su">/</abbr> 30</span></div></div><label class="form-label" for="add-comment-text">Commenta</label><textarea id="add-comment-text" maxlength="200" placeholder="Motiva la tua valutazione" required="required" name="comment"></textarea><p class="err-msg"><button class="button-input button-input-confirm" type="submit" name="submit-add-review">VALUTA</button><button class="button-input button-input-cancel" type="reset">CANCELLA</button></fieldset></form></div>';
            }    
