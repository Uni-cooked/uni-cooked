    ...
    if($isUserLogged!=false)
    {
        ...
        if(is_string($userComment) && strcmp($userComment,"userLeftNoComment")==0) {
            ...
            if(isset($_SESSION["commentError"])) {
                $form.='<form method="post" action="recipe.php?recipe='.urlencode($recipe).'#eval-section" class="content" id="first-form">'.'<fieldset><div class="input-container"><label class="form-label" for="add-comment-eval">Valuta questa ricetta</label><p class="suggestion">Voto da 1 a 30</p><div id="add-comment-eval-container"><input type="number" id="add-comment-eval" min="1" max="30" placeholder="18" required name="mark"/><span><abbr title="su">/</abbr> 30</span></div></div><label class="form-label" for="add-comment-text">Commenta</label><p class="suggestion">Massimo 200 caratteri</p><textarea id="add-comment-text" maxlength="200" placeholder="Motiva la tua valutazione" required name="comment"></textarea><p class="err-msg">'.$_SESSION["commentError"].'<button id="btn-confirm" class="button-input button-input-confirm" type="submit" name="submit-add-review">VALUTA</button><button class="button-input button-input-cancel" type="reset">CANCELLA</button></fieldset></form></div>';
                unset($_SESSION["commentError"]);
            } else {
                $form.='<form method="post" action="recipe.php?recipe='.urlencode($recipe).'#eval-section" class="content" id="first-form">'.'<fieldset><div class="input-container"><label class="form-label" for="add-comment-eval">Valuta questa ricetta</label><p class="suggestion">Voto da 1 a 30</p><div id="add-comment-eval-container"><input type="number" id="add-comment-eval" min="1" max="30" placeholder="18" required name="mark"/><span><abbr title="su">/</abbr> 30</span></div></div><label class="form-label" for="add-comment-text">Commenta</label><p class="suggestion">Massimo 200 caratteri</p><textarea id="add-comment-text" maxlength="200" placeholder="Motiva la tua valutazione" required name="comment"></textarea><p class="err-msg"><button id="btn-confirm" class="button-input button-input-confirm" type="submit" name="submit-add-review">VALUTA</button><button class="button-input button-input-cancel" type="reset">CANCELLA</button></fieldset></form></div>';
            }    
        ...
        $paginaHtml=str_replace("{{leave-comment-or-personal-published-comment}}",'<p id="no-login-p"><a href="sign-in.php?ref='.urlencode($recipe).'">Accedi</a> per poter commentare!</p>',$paginaHtml);
    }
